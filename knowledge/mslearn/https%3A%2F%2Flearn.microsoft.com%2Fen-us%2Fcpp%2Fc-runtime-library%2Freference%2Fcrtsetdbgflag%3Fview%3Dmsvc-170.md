...
Number of `malloc`, `realloc`, `free`, and `_msize` calls between calls to `_CrtCheckMemory`
...
3.  Turn off the other bits with a bitwise "and" (`&`) of the variable with a bitwise "not" (`~`) of the appropriate bitmasks.
